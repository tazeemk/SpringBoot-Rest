...
public class TouristRestController 
{
   ...
   public ResponseEntity<List<Tourist>>getTourist(){
	...
	/*
	 * @GetMapping("showone/{sno}") public
	 * ResponseEntity<?>getTourById(@PathVariable("sno")Integer no){ try { Tourist
	 * tour =tourService.showById(no); return new
	 * ResponseEntity<Tourist>(tour,HttpStatus.OK); }catch(Exception e) { return new
	 * ResponseEntity<String>(e.getMessage(),HttpStatus.INTERNAL_SERVER_ERROR); } 
	 * }
	 */ 
   
   @GetMapping("showone/{sno}")
   public ResponseEntity<Tourist>getTourById(@PathVariable("sno")Integer no){
